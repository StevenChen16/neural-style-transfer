...
def parse_opt(known=False):
    ...
    parser.add_argument(
        "--content_img_path", type=str, default="./images/1.jpg", help="原图路径"
    )
    parser.add_argument(
        "--style_img_path", type=str, default="./images/style.jpg", help="风格图片路径"
    )
    parser.add_argument(
        "--output_path", type=str, default="./output/1", help="生成图片保存路径"
    )
    ...
    parser.add_argument(
        "--style_loss_factor", type=int, default=100, help="风格loss总加权系数"
    )
    parser.add_argument(
        "--img_size",
        type=int,
        default=0,
        help="图片尺寸,0代表不设置使用默认尺寸(450*300),输入1代表使用图片尺寸,其他输入代表使用自定义尺寸",
    )
